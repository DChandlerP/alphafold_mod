...
# copy non-docker script to /app/alphafold
RUN wget https://github.com/kalininalab/alphafold_non_docker/blob/main/run_alphafold.sh
RUN chmod +x run_alphafold.sh

ENV PATH=/opt/conda/bin:/usr/local/cuda/bin/:/app/alphafold:$PATH \
    LC_ALL=C.UTF-8 \
    ALPHAFOLD_BASE_DIR=/app/alphafold

# try to avoid mounting $HOME/etc
RUN chmod -R 777 /etc
